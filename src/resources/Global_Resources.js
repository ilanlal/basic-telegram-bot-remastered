const Global_Resources = {
    en: {
        appDescription: "Json Studio is json tools for Google Sheets™ to format, minify, and validate JSON data.",
        messages: {
            tip: "Tip!",
            deprecated: "This feature is deprecated and may be removed in future versions.",
            tipForMenu: "Use the menu to access the JSON tools.",
            failure: " ❌ Operation failed.",
            error: " 🚨 An error occurred.",
            success: " ✅ Operation completed successfully.",
            operationSuccess: " ✅ Operation completed successfully. {0} cell(s) processed.",
            info: " 💡 Information",
            warning: " ⚠️ Warning",
            confirm: " ⚠️ Are you sure you want to proceed?",
            loading: " ⏳ Loading, please wait...",
            noData: "No data available.",
            invalidJson: "📛 Invalid JSON data.",
            rangeNotSelected: "Please select a range of cells containing JSON data.",
            rangeSelected: "Range selected successfully.",
            notSupported: "This operation is not supported in the current context.",
            dialogClosed: "Dialog closed.",
            addOnOnly: "This add-on can only be used in Google Sheets™.",
            errorParsingJson: "Error parsing JSON data.",
            errorMinifyingJson: "Error minifying JSON data.",
            outOfRange: "The selected range {0} exceeds the maximum allowed size of {1} cells. Please select a smaller range.",
            errorPrettifyingJson: "Error prettifying JSON data.",
            cell: "Cell",
            cellUpdated: "Cell updated successfully.",
            cellCleared: "Cell cleared successfully.",
            totalCellsProcessed: "Total cells processed: {0}",
            totalCellsUpdated: "Total cells updated: {0}",
            totalCellFailed: "Total cells failed: {0}",
            authRequired: "Authentication is required to perform this operation.",
            premiumRequired: "Premium membership required.",
            upgradeRequired: "Upgrade required to access this feature.",
            premiumFeature: "This is a premium feature. Please activate your premium license to use it.",
            premiumActivated: "Premium Activated",
            claimPremium: "🎁 Claim your free {0} days premium membership now!",
            focusedOnCell: "Focused on cell: {0}",
            packageInfo: "Add-On Information",
            profileInfo: "Profile Information",
            accountInfo: "Account Information",
            settingsInfo: "Settings Information",
            membershipInfo: "Membership Information",
        },
        menu: {
            top: "Json studio",
            format: "Format",
            minify: "Minify",
            prettify: "👁️ Prettify",
            edit: "✏️ Edit",
            help: "Help",
            support: "Support",
            about: "About",
            settings: "⚙️ Settings",
            easyAdm: "Easy ADM",
        },
        cards: {
            botSetup: {
                title: "Bot Setup",
                hint: "Enter your Telegram Bot Token",
                subtitle: "Configure your bot settings",
                description: "Use this section to set up and configure your bot.",
                imageAltText: "Json Studio Bot Setup Card",
            },
            account: {
                title: "Account",
                subtitle: "Manage your account settings",
                description: "Use this section to manage your account settings and preferences.",
                imageAltText: "Json Studio Account Card",
                license: "Membership : {0}",
                activationInstructions: "To activate your premium membership, click the button below.",
                activationSuccess: "Your premium membership has been activated successfully.",
                activationError: "An error occurred while activating your premium membership. Please try again later.",
                activationButton: "Activate Premium",
            },
            home: {
                title: "Welcome to Json Studio",
                subtitle: "Formatted JSON in Google Sheets™",
                content: "Use the buttons below to format JSON data in your spreadsheet.",
                tip: "💡 Select a range of cells containing JSON data and use the buttons below to format or minify the JSON.",
                imageAltText: "Json Studio for Google Sheets™",
                footer: "Use the buttons below to format (prettify) or minify JSON data in your spreadsheet.",
                formatAlt: "Prettified JSON.",
                formatTip: "Select a range of cells containing JSON data and click the 'Format' button to prettify the JSON.",
                minifyAlt: "Minified JSON.",
                minifyTip: "Select a range of cells containing JSON data and click the 'Minify' button to minify the JSON.",
                editAlt: "Open in Json Editor",
                editTip: "Select a cell containing JSON data and click the 'Edit' button to open the JSON Editor.",
                advanced: "Advanced",
                moreOptions: "More Options",
                moreOptionsContent: "Select a <b>range</b> of cells containing JSON data and use the buttons below to format (prettify) or minify the JSON.",
                indentSpaces: "Indentation Spaces",
                indentSpacesContent: "Set your custom indentation spaces for prettifying JSON (default is 2)",
                tools: "Tools",
            },
            report: {
                title: "Parse failed",
                subtitle: "Total {0} issue(s) found.",
                imageAltText: "Json Studio Report Card",
                sectionHeader: "Issues:",
                noIssuesFound: "No issues found.",
                summary: "Summary of issues"
            },
            about: {
                title: "ℹ️ About",
                subtitle: "About Json Studio",
                content: "Json Studio is a powerful tool for working with JSON data in Google Sheets. It allows you to format, minify, and edit JSON data easily.",
                imageAltText: "Json Studio for Google Sheets™",
            }
        },
        actions: {
            format: "Format",
            minify: "Minify",
            prettify: "Prettify",
            edit: "Edit",
            help: "Help & Support",
            about: "About Json Studio",
            settings: "Settings",
            close: "Close",
            reset: "Reset Settings",
            save: "Save Settings",
            apply: "Apply",
            cancel: "Cancel",
            cancelLicense: "Cancel License",
            clear: "Clear",
            ok: "OK",
            confirm: "Confirm",
            yes: "Yes",
            no: "No",
            done: "Done",
            continue: "Continue",
            selectRange: "Select Range",
            refresh: "Refresh",
            reload: "Reload",
            add: "Add",
            claim: "Claim Premium",
            claimFreeXDays: "🎁 Claim a Free {0} Days",
            activate: "Activate",
            activatePremium: "🏆 Activate Premium",
            cancelPremium: "🔒 Cancel Premium",
            premiumIcon: "🏆",
            lockIcon: "🔒",
            unlockIcon: "🔓",
            freeIcon: "🆓",
            change: "Change",
            indentSpaces: "Indentation Spaces",
            focus: "👁️ {0}",
        }
    }
}